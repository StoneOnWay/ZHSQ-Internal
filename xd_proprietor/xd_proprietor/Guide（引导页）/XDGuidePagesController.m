...
@interface XDGuidePagesController ()<UIScrollViewDelegate>

@property (nonatomic, strong) UIPageControl *pageControl;

@end

@implementation XDGuidePagesController
...
- (void)guidePageControllerWithImages:(NSArray *)images {
    ...
    for (NSInteger i = 0; i < images.count; i ++) {
        [gui addSubview:({
            ...
            self.btnEnter.contentHorizontalAlignment = UIControlContentHorizontalAlignmentFill;
            self.btnEnter.contentVerticalAlignment = UIControlContentHorizontalAlignmentFill;
            self.btnEnter.imageView.contentMode = UIViewContentModeScaleAspectFill;
            self.btnEnter.imageView.clipsToBounds = YES;
            if (i == images.count - 1) {
...
//                btn.frame = CGRectMake(s_w * i, s_h - 60, 150, 30);
...
+ (BOOL)shouldShow {
    ...
    if (localVersion == nil || ![currentVersion isEqualToString:localVersion]) {
        [XDGuidePagesController saveCurrentVersion];
        return YES;
    } else {
        return NO;
    }
