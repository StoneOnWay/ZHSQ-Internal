...
@interface XDDragCellCollectionView ()
...
@implementation XDDragCellCollectionView
...
- (void)handelCellClick:(NSNotification *)notification {
    ...
    XDServerConfigCell *cell = notification.object;
    NSIndexPath *indexPath = [self indexPathForCell:cell];
    XDServerDataManager *manager = [XDServerDataManager shareDataManager];
    ...
    if (indexPath.section == 0) {
        ...
        if (manager.headArray.count == 8) {
            [XDUtil showToast:@"我的应用最大个数为8"];
            return;
        }
...
- (void)setBeginEditing:(BOOL)beginEditing {
    ...
    [XDServerDataManager shareDataManager].isEditing = beginEditing;
    [self reloadData];
