...
- (void)layoutSubviews
{
    ...
    if (!self.zoomType) {
        ...
        _flowLayout.itemSize = CGSizeMake(_maxWidth, _maxHeight);
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.zoomType) {
        if ([self currentIndex] == indexPath.item) {
            cell.imageView.frame = CGRectMake(0, 0, _maxWidth, _maxHeight);
        } else {
            cell.imageView.frame = CGRectMake(0, 0, (_maxWidth-50), (_maxHeight-30));
        }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (self.zoomType) {
        ...
        currentCell.imageView.frame = CGRectMake(0, 0, _maxWidth - scale * (_maxWidth - (_maxWidth-50)), _maxHeight - scale * (_maxHeight - (_maxHeight-30)));
        ...
        if (self.mainView.contentOffset.x > self.contentOffsetX) {
            ...
            if (self.flowLayout.itemSize.width * itemIndex < self.mainView.contentOffset.x + (Screen_Width - self.flowLayout.itemSize.width)/2) {
                // 当前页图片缩小，下一页图片放大
                nextCell.imageView.frame = CGRectMake(0, 0, (_maxWidth-50) + scale * (_maxWidth - (_maxWidth-50)), (_maxHeight-30) + scale * (_maxHeight - (_maxHeight-30)));
            } else {
                // 当前页图片放大，前一页图片缩小
                lastCell.imageView.frame = CGRectMake(0, 0, _maxWidth - (1 - scale) * (_maxWidth - (_maxWidth-50)), _maxHeight - (1 - scale) * (_maxHeight - (_maxHeight-30)));
            }
        } else {
            // 向右滚动
            if (self.isDragging) {
